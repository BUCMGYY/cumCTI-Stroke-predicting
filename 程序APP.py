...
feature_ranges = {
    ...
    'CTI control levels': {'type': 'categorical', 'options': [0.0, 1.0, 2.0, 3.0], 'default': 2.0},
    'depression category': {'type': 'categorical', 'options': [0.0, 1.0], 'default': 1.0},
    'social isolation': {'type': 'categorical', 'options': [0.0, 1.0], 'default': 0.0},
    'marital status': {'type': 'categorical', 'options': [0.0, 1.0, 2.0], 'default': 2.0},
    'sleep duration': {'type': 'categorical', 'options': [0.0, 1.0, 2.0], 'default': 1.0},
    'regional category': {'type': 'categorical', 'options': [0.0, 1.0, 2.0], 'default': 1.0},
    'cooking fuel use': {'type': 'categorical', 'options': [0.0, 1.0], 'default': 1.0},
    'residence': {'type': 'categorical', 'options': [0.0, 1.0], 'default': 0.0},
    'education status': {'type': 'categorical', 'options': [0.0, 1.0], 'default': 0.0},
    'stomach digestive disease': {'type': 'categorical', 'options': [0.0, 1.0], 'default': 0.0}
}
...
if st.button("Predict"):
    ...
    # ===== SHAP（兼容二分类/多分类两种返回格式）=====
    Xt = preprocess.transform(X_user)
    from scipy import sparse as _sp
    if _sp.issparse(Xt):
        Xt = Xt.toarray()

    # 获取 one-hot 后特征名（失败则用 f0..fn）
    try:
    ...
    # 解释器：用概率空间，结果更直观
    try:
        explainer = shap.TreeExplainer(rf, feature_perturbation="interventional", model_output="probability")
    except TypeError:
        explainer = shap.TreeExplainer(rf)  # 回退

    shap_values = explainer.shap_values(Xt)
    expected = explainer.expected_value

    # --- 兼容：二分类可能返回 ndarray (n_samples, n_features)，多分类返回 list ---
    if isinstance(shap_values, list):
        sv_row = shap_values[pos_idx][0]  # 当前展示的类别
        base = expected[pos_idx] if isinstance(expected, (list, np.ndarray)) else expected
    else:
        sv_row = shap_values[0]           # 二分类单输出
